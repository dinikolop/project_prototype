...
function customCheckboxes(cb, datefilter, maxcb, callback) {
  ...
  if (cb.checked) {
    ...
    var final_term = categories + datefilter + " AND " + search_terms + ' NOT pubstatusaheadofprint';
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      for (var j = 0; j < allmesh.length; j++) {
        ...
        if (icd10s) {
          ...
          if(!(singlePMIDdict['pmid'] in icd10scores[code][key+'-pubs'])) {
            icd10scores[code][key+'-pubs'].push(singlePMIDdict['pmid'])
          }
        }
      }
      ...
      for (var k=0; k<supplMesh.length; k++) {
        var smesh = supplMesh[k]['s_ui'];
        var icd10s = all_maps[0][smesh];
        if (icd10s) {
          for (code in icd10s) {
            var icd10name = all_maps[3][code]
            if (!icd10scores[code]) {
